...
output: 
  rjdistill::rjournal_web_article: default
...
```{r}
library(crosstalk)
shared_penguins <- SharedData$new(penguins, ~species)

simpson_nospecies <- ggplot(data = shared_penguins, aes(x = bill_length_mm, y = bill_depth_mm)) +
...
simpson_wspecies <-
  ggplot(data = shared_penguins, aes(x = bill_length_mm, y = bill_depth_mm, group = species)) +
...
bscols(widths = c(2,NA),
  list(
    filter_checkbox("species", "Species", shared_penguins, ~species, inline = TRUE)
  ),
  plotly::ggplotly(simpson_wspecies)
)
```
