...
abstract: > 
 ...
 In 1935, Edgar Anderson collected size measurements for 150 flowers from three different species of Iris on the Gaspé Peninsula. Since then, Anderson's *Iris* dataset has become a classic in statistics, machine learning, and data science teaching materials. It is included in the base R **datasets** package as **iris**, making it easy for users to access without knowing much about it. Most notably, Anderson's *Iris* dataset appeared in a 1936 publication by R. A. Fisher in the *Annals of Eugenics*, linking the dataset to a field of research rooted in racism and prejudice. Combined with the lack of dataset documentation, the presence of non-intuitive variables like "sepal width", and the perfectly balanced groups with zero missing values, *Iris* is an unsatisfactory dataset for teaching and illustrating concepts. Users would benefit from working with a more representative, real-world environmental dataset---one with a clean history and a clear link to modern scientific research. Thus, a modern alternative to the *Iris* dataset is needed. Here, we introduce the **palmerpenguins** R package, which includes morphological measurements for three species of *Pygoscelis* penguins that breed on islands throughout the Palmer Archipelago, Antarctica. The **penguins** dataset provides an approachable, charismatic, and near drop-in replacement for the **iris** data. We directly compare the two datasets for selected analyses to demonstrate that R users, in particular teachers and learners currently using the **iris** dataset, can switch to the Palmer Archipelago penguins for many use cases including data wrangling and visualization, correlation, regression, hypothesis testing, multivariate analysis (e.g., PCA), cluster analysis and classification (e.g., by k-means). 
...
Given the need for fresh data science-ready datasets, we sought to identify a suitable alternative dataset that could be made easily accessible for a broad audience. After evaluating the positive and negative features of the *Iris* dataset in data science and statistics materials, we established the following criteria for a suitable alternative:

- Available by appropriate license (ideally, CC0 "no rights reserved")
- Features intuitive subjects and variables that are interesting and understandable to learners across disciplines
...
The **palmerpenguins** R package contains two data objects: **penguins_raw** and **penguins**. The **penguins_raw** data consists of all raw data for `r ncol(palmerpenguins::penguins_raw)` variables (Appendix Table 1), recorded completely or in part for `r nrow(palmerpenguins::penguins_raw)` individual penguins, accessed directly from EDI. We generally recommend using the curated data in **penguins**, which is a subset of **penguins_raw** retaining all `r nrow(palmerpenguins::penguins)` observations, minimally updated (Appendix B) and reduced to the following eight variables:
...
Notably, while sample sizes in **iris** across species are all the same, sample sizes in **penguins** differ across the three species. The inclusion of three factor variables in **penguins** (*species*, *island*, and *sex*), along with *year*, create additional opportunities for grouping and faceting, compared to the single factor (*Species*) in **iris**.

Unlike **iris**, which contains only complete cases, the **penguins** dataset contains a small number of missing values (n~missing~ = 19, out of `r `ncol(palmerpenguins::penguins) * nrow(palmerpenguins::penguins)` total values). Missing values and unequal sample sizes are common in real-world data, and we believe add learning value to **penguins**. 
...
Both **penguins** and **iris** offer numerous opportunities to explore linear relationships and correlations, within and across species (Figures \ref{fig:penguins-pairs} & \ref{fig:iris-pairs}). A classic bivariate scatterplot made with the **iris** dataset reveals a clean linear relationship between petal length and petal width. Using **penguins** (\ref{fig:linear-example}), we can create a uniquely similar scatterplot with flipper length and body mass. The overall trend across all three species is approximately linear for both **iris** and **penguins**. Teachers may encourage students to explore how simple linear regression results and predictions differ when the species variable is omitted, compared to multiple linear regression with species included (\ref{fig:linear-example}).
...
Here, we have shown that structural size measurements for Palmer Archipelago *Pygoscelis* penguins, available as **penguins** in the **palmerpenguins** R package, offer a near drop-in replacement for **iris** in a number of common use cases for data science and statistics education including exploratory data visualization, linear correlation and regression, PCA, and clustering by k-means. In addition, teaching and learning opportunities in **penguins** are increased due to a greater number of variables, missing values, unequal sample sizes, and Simpson's paradox examples. Importantly, the **penguins** dataset encompasses real-world information derived from several charasmatic marine predator species with regional breeding populations notably responding to environmental change occuring throughout the Western Antarctic Peninsula region of the Southern Ocean (see Bestelmeyer et al. 2011, Gorman et al. 2014, 2015, 2017). Thus, the **penguins** dataset can facilitate discussions more broadly on biodiversity responses to global change - a contemporary and critical topic in ecology, evolution, and the environmental sciences.

Bestelmeyer, B.T., A.M. Ellison, W.R. Fraser, K.B. Gorman, S.J. Holbrook, C.M. Laney, M.D. Ohman, D.P.C. Peters, F.C. Pillsbury, A. Rassweiler, R. Schmitt, and S. Sharma. 2011. Analysis of abrupt transitions in ecological systems. Ecosphere 2(12):art129.
...
Gorman, K.B., T.D. Williams, and W.R. Fraser. 2014. Ecological sexual dimorphism and environmental variability within a community of Antarctic penguins (genus Pygoscelis). PLoS ONE 9(3): e90081. ***ALREADY REFERENCED
