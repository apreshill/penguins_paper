...
There are several reasons that practitioners and educators may want to retire Anderson's *Iris* dataset in modern data science materials, including: 

- Anderson's  *Iris* dataset entirely lacks metadata, limiting meaningful interpretation and discussion of research methods, analyses, and outcomes
- Two of its five variables, `Sepal.Width` and `Sepal.Length`, are not intuitive to most users
- In 1936, the *Iris* data were published by eugenicist R.A. Fisher in *Annals of Eugenics*, and are now inextricably linked to Fisher's eugenics work; in fact, the dataset is commonly referred to as *Fisher's Iris data*
- The *Iris* dataset contains only complete cases (i.e. no missing values) and equal sample sizes (*n* = 50 for each *Iris* species) - more perfect than real-world datasets learners are likely to encounter "in the wild"
- The single categorical variable (`Species`) limits options for analyses
...
Given these motivations to retire *Iris*, we sought to identify, then make readily available, a suitable alternative dataset. Considering *Iris* usage in data science and statistics materials, we established the following criteria for a suitable alternative:
