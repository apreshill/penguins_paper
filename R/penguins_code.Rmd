...
## Table 1: Overview summary of penguins and iris datasets

```{r, echo = TRUE}
# Features overview for penguins and iris: 
...
## Table 2: Counts comparison between iris and penguins

```{r compare_counts, echo = TRUE}

# Count iris observations by species:
iris_counts <- iris %>% 
  count(Species) %>% 
  rename(`Iris species` = Species)

# Count penguins observations by species and sex:
...
# Finalize combined counts table:
iris_penguin_n %>% 
...
## Figures 1 & 2: Overview comparison: numeric variables (pairs plots with ggpairs)
...
png(here("fig", "penguin_pairs.png"), units="in", width = 6, height = 4.5, res = 500)
...
png(here("fig", "iris_pairs.png"), units="in", width = 6, height = 4.5, res = 500)
iris_pairs <- iris %>%
  ggpairs(aes(color = Species, shape = Species),
...
## Figure 3: Example linear relationships
...
png(here("fig", "linear_example.png"), units="in", width = 6, height = 3, res = 500)
...
## Figure 6 and Table 3: K-means clustering
...
# Make cluster assignments table:
