...
\abstract{%
Edgar Anderson's famous \textbf{iris} dataset, which contains size
measurements for three species of \emph{Iris} collected in 1935, exists
in the base R \textbf{datasets} package as \textbf{iris} and is
ubiquitous in statistics and data science materials. Due to a lack of
metadata, non-intuitive variables (e.g.~Sepal.Width), its publication in
(and inextricable connection with) Fisher's 1936 paper in \emph{Annals
of Eugenics}, and perfect structure (e.g.~no missing values)
unrepresentative of most real-world environmental data, a modern
alternative to \textbf{iris} is needed. Penguin morphological
measurements for three species of \emph{Pygoscelis} penguins that breed
on islands throughout the Palmer Archipelago, Antarctica, provide an
approachable, charismatic and near drop-in replacement for the
\textbf{iris} data. Here, we introduce the \textbf{palmerpenguins} R
package and describe the included \textbf{penguins} data. We directly
compare the two datasets for selected analyses to demonstrate that R
users, in particular teachers and learners currently using the
\textbf{iris} data, can switch to the Palmer Archipelago penguins for
many use cases including data wrangling and visualization, correlation,
regression, hypothesis testing, multivariate analysis (e.g.~PCA),
...
There are a number of reasons that practitioners and educators may want
