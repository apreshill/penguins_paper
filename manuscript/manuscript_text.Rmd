...
For easier data access and use we created the [**palmerpenguins**](https://allisonhorst.github.io/palmerpenguins/) data package in *R*, which contains two data objects: 
...
Relationships, distributions and correlations can be explored for the four numeric variables in `iris` (sepal width and length, petal width and length) and `penguins` (flipper length, body mass, bill length and depth) in pair plots (Figure X A,B). 
...
ggsave(here("figures","penguins_ggpairs.png"), height = 6, width = 5)
...
ggsave(here("figures","iris_ggpairs.png"), height = 6, width = 5)
...
penguins_biplot <- pca_plot +
  ...
  geom_shadowtext(data = pca_wider,
            ...
            nudge_x = c(0.3,0.3,1.0,0.8), 
            nudge_y = c(-0.1,-0.2,0.1,-0.1),
            size = 3, 
            color = '#0A537D',
            bg.color = "white") +
  labs(title = "Penguins PCA biplot") +
...
penguin_screeplot <- penguin_percvar %>% 
  ...
  labs(title = "Penguins PCA screeplot") +
...
iris_biplot <- iris_pca_plot +
  ...
  geom_shadowtext(data = iris_wider,
            aes(x = PC1, y = PC2, label = terms), 
            nudge_x = c(0.3,0.3,0.7,0.6), 
            nudge_y = c(-0.1,-0.2,0.1,-0.1),
            size = 3, 
            color = '#0A537D',
            bg.color = "white") +
  labs(title = "Iris PCA biplot") +
...
iris_screeplot <- iris_percvar %>% 
  ...
  ylim(c(0,80)) +
  ylab("% of total variance") +
  labs(title = "Iris PCA screeplot") + 
  geom_text(aes(label = round(value,2)), vjust=-0.25) +
  theme(panel.background = element_rect(fill = "gray90", color = "gray90"))

(penguins_biplot | iris_biplot)/(penguin_screeplot | iris_screeplot)
...
#### k-means clustering: two variables

First, we compare k-means clustering using two variables for each dataset: for `iris`, petal width and petal length, and for a subset only including male `penguins`, bill length and bill depth. For each, we designate three centroids (*k* = 3) since there are three species of both *Iris* and penguins. 

Clustering by k-means with *Iris* petal dimension results in perfect separation for *Iris setosa* observations: all 50 *setosa* exist in a single cluster (cluster 1) with no *Iris virginica* or *versicolor* assigned to the cluster (Figure X, Table X). There is minimal species misclassification between the *versicolor* (cluster 2) and *versicolor* (cluster 3) species (Table X). 

Similarly, Adélie penguins are highly separated from Chinstrap and Gentoo penguins by k-means clustering using only bill length and bill depth: of 73 Adélie penguins in the data, 70 are assigned to a single cluster (Cluster 2), and only 3 are assigned to the cluster containing most Gentoos (Cluster 3). No Chinstraps or Gentoos are assigned to Cluster 2 (Figure X). The distinction between Chinstraps and Gentoos is less clear, in agreement with overlap in bill dimensions between the species observed visually (Figure X). 

```{r, echo = FALSE, message = FALSE, warning = FALSE, fig.align = "center"}
# TWO VARIABLE k-means comparison
# Male penguins: bill length vs. bill depth
...
combo_cluster <- cbind(bills_class, petal_class) %>% 
  kable(caption = "Table X. Assigned k-means clusters for results shown in Figure X") %>% 
  kable_styling(full_width = FALSE) %>% 
  add_header_above(c("Penguin bill cluster assignments" = 4, "Iris petal cluster assignments" = 4)) %>% 
  column_spec(c(5:8), background = "gainsboro")

combo_cluster
```

#### k-means clustering: all numeric variables

When all four numeric variables for each dataset are included in k-means clustering, *setosa* irises remain distinctly clustered apart from the other two iris species, but there is overlap across all penguin species (Table X). 

```{r, echo = FALSE, warning = FALSE, message = FALSE, fig.align = "center"}
# FOUR VARIABLE k-means comparison

penguins_k_full <- penguins %>% 
  select(species, bill_length_mm:body_mass_g) %>% 
  drop_na()

set.seed(1967)
kclust_penguins <- kmeans(penguins_k_full[,2:5], centers = 3, nstart = 20)


# Table of assigned clusters for penguins
penguins_class <- table(kclust_penguins$cluster,penguins_k_full$species)%>% 
  as.data.frame() %>% 
  rename(cluster = Var1, species = Var2, n = Freq) %>% 
  pivot_wider(names_from = species, values_from = n)
...
set.seed(1967)
iris_k_full <- kmeans(iris[,1:4], centers = 3, nstart = 25)

# Table of assigned clusters for iris petals: 
iris_class <- table(iris_k_full$cluster, iris$Species) %>% 
  as.data.frame() %>% 
  rename(cluster = Var1, Species = Var2, n = Freq) %>% 
  pivot_wider(names_from = Species, values_from = n)
...
# Full table of cluster assignments with all 4 numeric variables: 
combo_cluster_full <- cbind(penguins_class, iris_class) %>% 
  kable(caption = "Table X. Assigned k-means clusters for iris and penguins when including all four numeric variables for each") %>% 
  kable_styling(full_width = FALSE) %>% 
  add_header_above(c("Penguin cluster assignments" = 4, "Iris cluster assignments" = 4)) %>% 
  column_spec(c(5:8), background = "gainsboro")
...
## Differences by islands? 
>>>>>>> 0181779cd02ccdc2413d2698c7029f18572f3757

```{r}
hc_iris <- hclust(dist(iris[, 1:4]))
ggdendrogram(hc_iris, rotate = TRUE, size = 2)

p_cc <- penguins %>% 
  drop_na()

hc_penguins <- hclust(dist(p_cc[, 3:6]))
ggdendrogram(hc_penguins, rotate = TRUE, size = 2)
